...
import { Backdrop, Box, Button, Modal, TextField, Zoom } from "@mui/material"
...
const LoginModal = () => {
  const { showLoginModal, setShowLoginModal } = useWebModalStore()
  ...
  return (
    ...
    >
      <Zoom in={showLoginModal} unmountOnExit>
        <Box className='w-96 max-w-[100vw] absolute left-1/2 top-1/2 
          !-translate-x-1/2 !-translate-y-1/2 rounded-xl overflow-hidden shadow bg-white outline-none'
        >
          <div className="absolute right-4 top-4 z-10">
            <span 
              className="w-8 h-8 p-1.5 cursor-pointer flex items-center justify-center"
              onClick={handelClose}
            >
          ...
          >
            <div className="px-6 py-8">
              <h3 className="font-semibold text-center text-xl">{modal == "login" ? 'Đăng nhập hệ thống' : 'Đăng ký tài khoản'}</h3>
            ...
            <div className="bg-gray-50 border-t p-4 font-semibold rounded-b text-center">
              ...
              <button onClick={changeModal} className="ml-2 text-red-500">{modal == "register" ? 'Đăng nhập' : 'Đăng ký'}</button>
            </div>
          </form>
        </Box>
      </Zoom>
